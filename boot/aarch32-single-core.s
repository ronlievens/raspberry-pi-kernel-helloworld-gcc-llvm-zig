...
_start:
    ...
    // -----------------------------------------------------------------------------------------------------------------
    mrc p15, 0, r5, c0, c0, 5  // This command is a co-processor instruction used to access system control registers. It reads the system control register c0 in coprocessor p15 and stores the value into register r5. Note that p15 is generally used to refer to the system control processor. The last number 5 refers to the specific opcode transfer, signifying which function to perform on the coprocessor.
    and r5, r5, #3             // This bitwise 'and' operation is used to retain the least significant 2 bits of the value in register r5 and zero out the other bits.
    cmp r5, #0                 // This compare operation subtracts 0 from the value in r5 and sets the flags based on the result, but does not store the result. The flags that get set include zero (Z), negative (N), and carry (C).
    bne halt                   // This means branch if not equal. If the previous cmp operation found that r5 was not equal to 0, it will branch (i.e., jump) to the halt label in the program.
